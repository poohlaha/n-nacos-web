...
const ArticleList = (): ReactElement => {
  ...
  const render = () => {
    let list = noteStore.info?.list || []
    let archiveList = noteStore.info?.archiveList || []
    if (archiveList.length > 0) {
      archiveList = archiveList.slice(0, 7)
    }

    let tagList = noteStore.info?.tagList || []
    let tagClassifyList = noteStore.info?.tagClassifyList || []
    if (tagClassifyList.length > 0) {
      tagClassifyList = tagClassifyList.slice(0, 10)
    }
    let newDate = noteStore.new?.createTime || ''
    if (!Utils.isBlank(newDate || '')) {
      newDate = new Date(newDate.replace(/-/g, '/'))
    }

    let listCount = noteStore.info?.listCount || 0
      ...
      >
        <div className="article-content flex w100 min-h100 center pt-5">
          <div className="box-left flex-1 flex-direction-column">
            ...
            {listCount > noteStore.articlePageSize && (
              <div className="pagination flex-center page-margin-top">
                <Pagination
                  current={noteStore.currentPage}
                  total={listCount}
                  pageSize={noteStore.articlePageSize}
                  onChange={async (page: number, pageSize: number) => {
                    noteStore.currentPage = noteStore.articlePageSize !== pageSize ? 1 : page
                    // noteStore.pageSize = pageSize
                    await noteStore.getList()
                  }}
                />
              </div>
            ...
            <div className="new-list info-item page-margin-top">
              ...
              <div className="list-item-content">
                <div className="new-list item-content">
                  {noteStore.newList.length > 0 &&
                    noteStore.newList.map((item: { [K: string]: any } = {}, index: number) => {
                      return (
                        <div
                ...
                <div className="list-item-title new-list-title flex-jsc-between">
                  ...
                  <div className="list-item-title-right">
                    <div
                      ...
                      onClick={() => navigate(`${RouterUrls.NOTE.URL}${RouterUrls.NOTE.TAG.URL}`)}
                    >
                      <svg
                        className="svg-icon wh100"
                        viewBox="0 0 1024 1024"
                    ...
                    {archiveList.map((item: { [K: string]: any } = {}, index: number) => {
                      return (
                        <div
                          ...
                          onClick={() => {
                            navigate(
                              `${RouterUrls.NOTE.URL}${RouterUrls.NOTE.TAGDETAIL.URL}?monthName=${Utils.encrypt(
