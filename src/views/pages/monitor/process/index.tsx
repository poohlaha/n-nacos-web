...
const Index = (): ReactElement => {
  ...
  const getProcessHtml = (processes: Array<{ [K: string]: any }> = []) => {
    ...
    return processes.map((process: { [K: string]: any } = {}, index: number) => {
      return (
        <Descriptions bordered column={2} key={index}>
          <Descriptions.Item label="pid">
            <p className="color-text">{process.pid || ''}</p>
          </Descriptions.Item>
          <Descriptions.Item label="parentPid">
            <p className="color-text">{process.parent_pid || ''}</p>
          </Descriptions.Item>
          <Descriptions.Item label="start time">
            <p className="color-text">{process.start_time || ''}</p>
          </Descriptions.Item>
          <Descriptions.Item label="cmd">
            <p className="color-text">{process.cmd || ''}</p>
          </Descriptions.Item>
          <Descriptions.Item label="exe" className="description-exec">
            <p className="color-text">{process.exe || ''}</p>
          </Descriptions.Item>
          <Descriptions.Item label="virtual memory(bytes)">
            <p className="color-text">{process.virtual_memory || ''}</p>
          </Descriptions.Item>
          <Descriptions.Item label="run time">
            <p className="color-text">{process.run_time || ''}</p>
          </Descriptions.Item>
