...
const Server = (): ReactElement => {
  ...
  const {serverStore, mainStore} = useStore()
  ...
  const getChartOptions = (date: Array<any>, data: Array<any>, text: string = '', index: number, totalMem: number = 0) => {
    return {
      tooltip: {
        trigger: 'axis',
      },
        ...
        <Descriptions bordered column={2}>
          ...
          <Descriptions.Item label="密码">
            <div className="flex flex-align-center">
              <p className="color-text flex-1">
                {
                  showPwd ? serverStore.serverInfo?.pwd || '' : '*********'
                }
              </p>
          ...
          <Descriptions.Item label="描述">
            <p className="color-text" style={{whiteSpace: 'pre'}}>{serverStore.serverInfo?.desc || '-'}</p>
          </Descriptions.Item>
        </Descriptions>
      </Card>
    )
