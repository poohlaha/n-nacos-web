...
const PipelineAdd: React.FC<IRouterProps> = (props: IRouterProps): ReactElement => {
    ...
    const getBasicHtml = () => {
        ...
        if (!Utils.isBlank(pipelineStore.addForm.basic.tag || '')) {
            selectOptions.value = pipelineStore.addForm.basic.tag || ''
        }
    ...
    const getProcessHtml = () => {
        return (
            <div className="process-content wh100">
                <PipelineProcess processList={pipelineStore.activeProcess} isRun={false} />
            </div>
        )
    }
                ...
                <div className="content-box overflow flex-1">
                    <Tabs
                        ...
                        onChange={(activeKey: string = '') => {
                          /*
                          if (!Utils.isBlank(pipelineStore.addForm?.id)) {
                              return
                          }
                           */

                          if (activeKey === ADD_TABS[1].key) {
                            let basic = pipelineStore.addForm.basic || {}
                            let path = basic.path || ''
                            let isRemoteUrl = false
                            if (!Utils.isBlank(path)) {
                              isRemoteUrl = pipelineStore.isRemoteUrl(path)
                            }

                            if (isRemoteUrl) {
                              pipelineStore.activeProcess = pipelineStore.H5_REMOTE_TEMPLATE
                            } else {
                              pipelineStore.activeProcess = pipelineStore.H5_LOCAL_TEMPLATE
                            }
                          }
                        }}
                    />
